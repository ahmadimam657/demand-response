...
class VirtualStorage:
    ...
    release energy across time to minimize total purchasing costs while tracking
    a reference consumption profile. The storage size is constrained implicitly
    through max_demand_advance, max_demand_delay, and max_rate. There is therefore
    no need to model and constrain the storage level separately.
    ...
    Reference: [10, 15, 8, 12] kWh (default to demand)
    ...
    - Mutual exclusivity: Can either add to OR remove from storage during an
        hour, not both
    - Deviation balance: purchase[t] - reference[t] = pos_dev[t] - neg_dev[t]
    """
    ...
    def __init__(
        ...
        Args:
            ...
            enforce_charge_direction: If True, enforce mutual exclusivity between
                charging and discharging (default True). Setting to False allows
    ...
    def optimize_demand(
        ...
        down_price: np.ndarray | None = None,
        ...
        that minimizes total purchasing costs while tracking a reference profile.
        Uses the constraints on energy movement to ensure physical feasibility.
        ...
        Args:
            ...
            n_control_hours: Number of intervals to actively optimize.
                ...
                Must be <= len(demand).
            up_price: Upregulation price deviation from spot (ct/kWh).
                Typically ≥0. Payment received for consuming less than reference.
                Defaults to zeros if None.
            down_price: Downregulation price deviation from spot (ct/kWh).
                Typically ≤0. Discount received for consuming more than reference.
                Defaults to zeros if None.
            reference: Reference consumption profile to track (kWh).
                Defaults to demand if None. Deviations enable reserve market benefits.
        ...
        Returns:
            ...
            - "debug_info": Dictionary containing detailed optimization variables
              (only present if debug=True):
                ...
                - "pos_dev": Positive deviations from reference (kWh)
                - "neg_dev": Negative deviations from reference (kWh)
                - "charge_direction": Binary charge direction
                  (1=charging, 0=discharging)
                - "deviation_direction": Binary deviation direction
                  (1=positive deviation, 0=negative deviation)
        ...
        up_price = self._validate_optional_array(up_price, price.shape[0], "up_price")
        down_price = self._validate_optional_array(
            down_price, price.shape[0], "down_price"
        )

        # Auto-detect if deviation tracking is needed
        track_deviations = np.any(up_price != 0) or np.any(down_price != 0)

        # Reference only needed when tracking deviations
        if track_deviations:
            if reference is None:
                reference = demand.copy()
            else:
                reference = self._validate_optional_array(
                    reference, demand.shape[0], "reference"
                )
        else:
            # Not tracking deviations, set reference to demand to avoid None issues
            reference = demand
        ...
        variables = self._create_decision_variables(
            model, ranges, transfer_indices, reference, track_deviations
        )

        # Add objective function
        self._add_objective_function(
            model, variables, price, down_price, up_price, ranges, track_deviations
        )

        # Add constraints
        if track_deviations:
            self._add_deviation_constraints(model, variables, reference, ranges)
        ...
        if debug:
            debug_info = self._extract_debug_info(
                variables, ranges, transfer_indices, track_deviations
            )
    ...
    def _create_decision_variables(
        ...
        Args:
            ...
            reference: Reference consumption profile (kWh).
            track_deviations: If True, create deviation tracking variables.
        """
        ...
        # Deviation variables (only if tracking deviations from reference)
        if track_deviations:
            variables["deviation_direction"] = {
                t: s.add_var(model, f"dev_dir_{t}", vtype="binary")
                for t in ranges.local_time
            }
            variables["pos_dev"] = {
                t: s.add_var(
                    ...
                    f"pos_dev_{t}",
                    lb=0,
                    ub=self.max_hourly_purchase - reference[t],
                )
                for t in ranges.local_time
            }
            variables["neg_dev"] = {
                t: s.add_var(model, f"neg_dev_{t}", lb=0, ub=reference[t])
                for t in ranges.local_time
            }
    ...
    def _add_objective_function(
        ...
        """Minimize total cost: purchase*price + deviations*up/down prices.

        Implements cost minimization objective that penalizes total purchase costs
        plus deviations from a reference consumption profile (if tracking deviations).

        Args:
            model: The optimization model instance.
            variables: Dictionary of decision variables.
            price: Array of energy prices per interval (ct/kWh).
            down_price: Array of penalty prices for purchasing below reference (ct/kWh).
            up_price: Array of penalty prices for purchasing above reference (ct/kWh).
            ranges: TimeRanges object for time indexing.
            track_deviations: If True, include deviation terms in objective.
        """
        s = self._solver

        # Objective: minimize total cost
        if track_deviations:
            # Include deviation penalties
            objective = s.sum(
                [
                    variables["purchase"][t] * price[t]
                    + variables["pos_dev"][t] * down_price[t]
                    - variables["neg_dev"][t] * up_price[t]
                    for t in ranges.local_time
                ]
            )
        else:
            # Only minimize purchase cost
            objective = s.sum(
    ...
    def _add_deviation_constraints(
        ...
        """Add deviation tracking constraints.

        Enforces balance and mutual exclusivity between positive and negative
        deviations from the reference consumption profile.
        ...
        for t in ranges.local_time:
            # Balance: purchase - reference = pos_dev - neg_dev
            s.add_constraint(
                model,
                variables["purchase"][t] - reference[t]
                == variables["pos_dev"][t] - variables["neg_dev"][t],
                name=f"deviation_balance_{t}",
            )

            # Mutual exclusivity: only pos_dev OR neg_dev can be non-zero
            s.add_constraint(
                model,
                variables["pos_dev"][t]
                <= (self.max_hourly_purchase - reference[t])
                * variables["deviation_direction"][t],
                name=f"pos_dev_limit_{t}",
            )
            s.add_constraint(
                model,
                variables["neg_dev"][t]
                <= reference[t] * (1 - variables["deviation_direction"][t]),
                name=f"neg_dev_limit_{t}",
            )
    ...
    def _extract_debug_info(
        ...
        Args:
            ...
            ranges: TimeRanges object for time indexing.
            transfer_indices: TransferIndices object with computed mappings.
            track_deviations: If True, extract deviation variables.
        ...
        # Extract deviation variables only if tracking deviations
        if track_deviations:
            pos_dev = np.array(
                [s.get_value(variables["pos_dev"][t]) for t in ranges.local_time]
            )
            neg_dev = np.array(
                [s.get_value(variables["neg_dev"][t]) for t in ranges.local_time]
            )
            deviation_direction = np.array(
                [
                    s.get_value(variables["deviation_direction"][t])
                    for t in ranges.local_time
                ]
            )
        else:
